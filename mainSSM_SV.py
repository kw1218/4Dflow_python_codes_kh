...
## ---> SETTINGS
preprocDir = r'D:/InletProfileStudy/SSM/Output_kaihong/Patient_5_1100/'
...
for which_mode in range(0, 5): #first 5 modes
    ...
    for count, how_much_std in enumerate(b_range, start=0):
        ...
        outDirSynth = osp.join(synthOutDir, 'prof_mode{}_coeff_{}'.format(which_mode, how_much_std))
    ...
    for variation in range(total_flowrate.shape[1]):
        #ratio_SV =113 / SV_meanPlane   # Stroke volume
        ax.plot(x_value, total_flowrate[:, variation] , label=f"b={variation * 0.5 - 3}", color='black')
